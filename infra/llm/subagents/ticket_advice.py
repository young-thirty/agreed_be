...
async def _context(owner_id: PydanticObjectId, project_id: PydanticObjectId) -> tuple[str, dict[str, str]]:
    ...
    materials = (
        await ProjectMaterial.find(
            ProjectMaterial.ownerId == owner_id, ProjectMaterial.projectId == project_id
        )
...
async def build_ticket_advice(
    *,
    ...
    documents, shown = await _context(owner_id, project_id)
    task = (
        f"## 클라이언트 요청\n{summary_title}\n"
