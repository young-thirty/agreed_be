...
### 7-1. 회원가입 화면이 백엔드 세션을 만들지 않는다 (프론트 구현 예정)
...
**백엔드는 이 문서 2절 구조를 그대로 유지한다.** 프론트가 곧 `/api/auth/signup`과
`/api/auth/login`에 연결할 예정이므로 API를 바꾸지 않는다. 연결 전까지 시연은
브라우저에 미리 로그인해 둔 세션으로 한다.
...
**이을 때는 [DOMAIN_SPEC.md](./DOMAIN_SPEC.md) 7절의 이행 순서를 따른다.** 특히
2단계(후보 탐색 → 코드 확정 → LLM → manual_review)를 sync 안에 넣어야 채널
메시지가 프로젝트·티켓에 제대로 붙는다. 지금 sync는 source-link가 이미 프로젝트를
지정하고 있어 후보 탐색이 생략된 상태다.
...
### 7-4. 파일 업로드 카드가 동작하지 않는다 (채널 확장 예정)

연동 화면의 `파일 업로드`는 `사용 가능`으로 표시되지만 업로드 endpoint가 없다.
현재 `ProjectMaterial`은 Slack 첨부에서만 만들어진다. Gmail 첨부와 수동 업로드는
채널을 넓히는 단계에서 함께 붙인다. 원본은 S3에 저장되지만
