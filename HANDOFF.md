...
1. **DeepSeek 연결** — `infra/llm/client.py`는 완성돼 있고 키만 넣으면 된다. `.env`에 `DEEPSEEK_API_KEY` 설정 후 고정 시나리오가 아닌 대화로 확인해야 한다. **이게 유일하게 아직 실호출로 검증 안 된 부분이다.**
2. **슬랙 연동** — 담당자 있음. `infra/ingest/slack.py`를 만들어 `Utterance[]`를 뱉으면 나머지 파이프라인은 그대로 탄다.
3. **지메일 연동** — 같은 방식.
4. **계약서·제안서 대조** — `Basis`가 지금은 항상 `{"kind": "없음"}`이다. 계약서/제안서 원문과 대조하는 LLM 호출이 아직 없다. 시연 시나리오에서는 '근거 없음'이 정답이라 당장은 문제없다.
